...
- remember to delete a file the more convenient way is to use `git rm ...`
...
- if the file is already tracked you can use shortcut `git commit -am 'your message here'` to not add is separately 
...
- delete UNTRACKED directories `git clean -n -d` to discover what will be deleted (functions a lot like "dry run"), `git clean -f -d` delete the directories even if nonempty